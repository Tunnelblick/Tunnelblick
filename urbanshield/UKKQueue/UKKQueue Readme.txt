...
0.4 - Now includes UKFNSubscribeFileWatcher (doesn't quite work, but you can use it to notify the Finder), and closes the kqueue file descriptor in a separate thread (thanks to Dominic Yu for the suggestion!) so you don't have to wait for close() to time out.
