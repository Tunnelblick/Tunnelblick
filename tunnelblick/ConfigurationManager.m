...
+(void) renameSafeConfiguration: (NSDictionary *) dict {
    ...
    // Rename the private copy
    if (  ! [gFileMgr tbForceRenamePath: sourcePath toPath: targetPath]  ) {
        goto err;
    }

    // Rename the secure (shadow) copy
    NSArray * arguments = @[@"safeRename", sourceName, targetName];
    int status = runOpenvpnstart(arguments, nil, nil);
    if (  status != 0  ) {
        // Undo rename of private copy
        [gFileMgr tbForceRenamePath: targetPath toPath: sourcePath];
        goto err;
    }

    // Rename the credentials
    if (  ! [self moveOrCopyCredentialsAndSettingsFrom: sourceName to: targetName moveNotCopy: YES]  ) {
        // Undo rename of secure (shadow) copy and private copy
        arguments = @[@"safeRename", targetName, sourceName];
...
+(void) renameConfigurationWithPathsOperation: (NSDictionary *) dict {
    ...
    if (  [self isSafeConfigurationForDisplayName: sourceName]  ) {
        ...
        [gMC performSelectorOnMainThread: @selector(renameSafeConfigurationUsingConfigurationManager:) withObject: dict2 waitUntilDone: YES];
        goto done;
    }
