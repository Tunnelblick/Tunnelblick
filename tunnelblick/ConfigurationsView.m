...
-(void) awakeFromNib {
	...
	NSAttributedString * infoTitle = attributedStringFromHTML(NSLocalizedString(@"<p><strong>When checked</strong>, Tunnelblick will watch for and react to changes in network"
																				@" settings (for example, changes caused by DHCP renewals). Tunnelblick's default actions when network"
	...
	infoTitle = attributedStringFromHTML(NSLocalizedString(@"<p><strong>When checked</strong>, Tunnelblick will force OpenVPN to route all IPv4 traffic through the VPN server.</p>\n"
														   @"<p><strong>When not checked</strong>, by default OpenVPN will use the VPN only for traffic that"
														   @" is destined for the VPN's network. All other traffic will use the normal Internet connection without going through the VPN.</p>\n"
														   @"<p>Note that this default behavior may be changed by the VPN server or the OpenVPN configuration and cause all IPv4"
														   @" traffic to be routed through the VPN server as if this <em>had</em> been checked. </p>\n",
	...
	infoTitle = attributedStringFromHTML(NSLocalizedString(@"<p><strong>When checked</strong>, IPv6 will be disabled.</p>\n"
														   @"<p>This is often recommended because many VPN configurations do not guard against information leaks caused by the use"
														   @" of IPv6 and most Internet access works fine without IPv6.</p>\n"
